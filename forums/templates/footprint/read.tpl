...
{LOOP MESSAGES}
  ...
  <div class="PhorumReadMessageBlock">
    ...
    <div class="PhorumStdBlock">
      ...
      <div class="PhorumReadBodyHead">{LANG->Postedby}: <strong>{MESSAGES->linked_author}</strong> {IF MODERATOR true}({MESSAGES->ip}){/if}</div>
    ...
    <div class='forumOptions'>
      <a href="{MESSAGES->reply_url}">{LANG->Reply}</a> | 
	  <a href="{MESSAGES->quote_url}">{LANG->QuoteMessage}</a>
	  
	  {IF LOGGEDIN}
	  	{IF MESSAGES->private_reply_url}
	  		&bull;<a class="PhorumNavLink" href="{MESSAGES->private_reply_url}">{LANG->PrivateReply}</a>
	  	{/IF}
	  	&bull;<a class="PhorumNavLink" href="{MESSAGES->follow_url}">{LANG->FollowThread}</a>
	  	&bull;<a class="PhorumNavLink" href="{MESSAGES->report_url}">{LANG->Report}</a>
	  {/IF}
	  
	  {IF MESSAGES->edit 1}
	  	&bull;<a class="PhorumNavLink" href="{MESSAGES->edituser_url}">{LANG->EditPost}</a>
	  {/IF}
...
<div class="PhorumNavBlock" style="text-align:right;">
    <a href="{URL->NEWERTHREAD}">&laquo; {LANG->PrevPage}</a> | <a href="{URL->OLDERTHREAD}">{LANG->NextPage} &raquo;</a>
</div>
