...
$smarty->assign('page'          , array('documentNew' => true));
...
else
{
	...
	if($valid=SmartyValidate::is_valid($_POST))
	{
		...
		$fp->doc = new document;
		$newID = $fp->doc->save($formVars, $_FILES['file']);
		if(is_numeric($newID)) $success = true;
		
		// Update Activity Log
		if($success) $fp->updateActivityLog('documentNew', $newID);
	}
...
// Display in Template
$smarty->display('documentNew.tpl');
?>